...
class ShippingAddress: UIViewController ,UITableViewDelegate,UITableViewDataSource
{
    ...
    override func viewDidLoad()
    {
        ...
        self.tabVlist.register(UINib(nibName: "SACell", bundle: nil), forCellReuseIdentifier: "SACell")
        ...
        border.frame = CGRect(x: 0, y: btnShipping.frame.size.height - width, width: btnShipping.frame.size.width, height: btnShipping.frame.size.height)
        border.borderWidth = width
        btnShipping.layer.addSublayer(border)
        btnShipping.layer.masksToBounds = true
    }
    ...
    {
        let cell = tableView.dequeueReusableCell(withIdentifier: "SACell", for: indexPath) as! SACell
    ...
    //MARK: - get ShippingAddress method
    ...
    func setShippingAddress(strAddressId:String)
    {
        ...
        let strconnurl = String(format: "%@%@", Constants.conn.ConnUrl, "api/client/SelectShippingAddress")
        let request = NSMutableURLRequest(url: NSURL(string: strconnurl)! as URL)
        request.httpMethod = "POST"
        request.setValue(strapikey, forHTTPHeaderField: "apiSecretKey")
        request.setValue("application/json", forHTTPHeaderField: "Content-Type")
        ...
        let task = URLSession.shared.dataTask(with: request as URLRequest){ data, response, error in
            ...
            do{
                if let json = try JSONSerialization.jsonObject(with: data!) as? NSDictionary
                {
                    ...
                    let arrm1 :NSArray =  json.value(forKey: "ShippingMethods") as! NSArray
                    ...
                    OperationQueue.main.addOperation {
                        ...
                        ShippingMethod.arrMShippingMethod = NSMutableArray(array: arrm1)
                        ShippingMethod.hidesBottomBarWhenPushed = true
                        self.navigationController?.pushViewController(ShippingMethod, animated: false)
